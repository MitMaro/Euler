...
int main (void) {
	...
	int triangle[] = \
		{75, 95, 64, 17, 47, 82, 18, 35, 87, 10, 20, 4, 82, 47, 65, 19, 1, 23, 75, 3, 34, 88, 2, 77, 73, 7, 63, \
	...
	for (i = 13; i >= 0; --i) {
		...
		depth_sum = (i*i + i)/2 + i + 1;
		
		for (j = 0; j < i+1; ++j) {
			if (triangle[depth_sum + j] > triangle[depth_sum + j + 1]) {
				triangle[depth_sum - i - 1 + j] += triangle[depth_sum + j];
			} else {
				triangle[depth_sum - i - 1 + j] += triangle[depth_sum + j + 1];
			}
		}
	}
	printf("Answer: %d\n", triangle[0]);
